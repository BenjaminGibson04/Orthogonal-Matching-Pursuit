...
Let $$A\in\mathbb{R}^{m\times n}$$, $$b\in \mathbb{R}^m$$ for the system $$Ax=b$$. Let $$x_0 = 0, r_0 = b, S_0=\emptyset, k=0$$. For stopping criteria, run the while loop 1000 times. The general outline for the OMP is as follows:

while number of iterations $$k \leq 1000 $$:
...
* $`(x_{k+1})_{S_{k+1}} = A^+_{S_{k+1}}b, (x_{k+1})_{S^c_{k+1}} = 0`$, where $`A^+`$ is the pseudo-inverse of $`A`$. $`x_{k+1}`$ is a vector here.
